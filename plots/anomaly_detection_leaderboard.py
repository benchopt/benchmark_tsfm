...
Columns: Rank | Solver | AUC-PR ↑ | AUC-ROC ↑ | F1-PA ↑

All values are mean across datasets. Sorted by AUC-PR descending.
"""
...
_METRICS: list[tuple[str, str, bool]] = [
    ("objective_auc_pr",  "AUC-PR",  False),  # False = higher is better
    ("objective_auc_roc", "AUC-ROC", False),
    ("objective_f1_pa",   "F1-PA",   False),
]
...
class Plot(BasePlot):
    """Anomaly detection leaderboard: all AD metrics, sorted by AUC-PR."""
    ...
    def plot(self, df):
        ad_cols = [c for c, _, _ in _METRICS if c in df.columns]
        if not ad_cols:
            return [["(no anomaly detection data)", *["—"] * len(_METRICS)]]
    ...
    def get_metadata(self, df):
        ...
        return {
            "title": "Anomaly detection leaderboard — mean metrics across datasets",
            "columns": columns,
        }
